...
colunas <- c(unique(sort(substr(dadosRespostasComunidade$dataCriacao, 1, 7))), "SOMA_VOTE_DOWN", "MEDIA" , "REPUTACAO")
...
for(i in 1:nrow(top50DadosRespostas)) {
  ...
  votesDown <- row$votosNegativos
  if(ano >= 2012) {
    tabela[usuario, anoMes] <- tabela[usuario, anoMes] + votesDown
  } 
}
...
for(usuario in usuarioList) {
  for(coluna in colunas) {
    if(coluna != "SOMA_VOTE_DOWN" & coluna != "REPUTACAO" & coluna != "MEDIA") {
      tabela[usuario, "SOMA_VOTE_DOWN"] <- tabela[usuario, coluna] + tabela[usuario, "SOMA_VOTE_DOWN"]  
    }
  }
  tabela[usuario, "MEDIA"] <- (tabela[usuario, "SOMA_VOTE_DOWN"]/(length(colunas) - 3))
...
boxplot(medias,  ylab = "Average Votes Down")
print(summary(medias))
write.table(medias, file = "mediasTop50VoteDown.csv", sep = ";")